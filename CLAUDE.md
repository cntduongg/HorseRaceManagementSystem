...
| `Tournament` | Giải đấu (tên, venue, ngày). `StartDate`/`EndDate` là **`DateOnly`**; ⚠️ **`LogoUrl` đã bị gỡ 2026-07-29** (migration `DropTournamentLogoUrl`) — không còn logo giải ở cả FE lẫn BE; `Status` (Draft\|Open\|Ongoing\|Finished\|Cancelled) nay có hằng số `Domain/Aggregates/Constants/TournamentStatus.cs` + **tự chuyển theo ngày** qua `TournamentStatusSyncBackgroundService`. (Flow 3) |
...
- **Auth** (`Api/Controllers/AuthController.cs` + `Application/Usecases/Auth/`):
  - `POST /api/auth/register/{spectator|horse-owner|jockey}` — Spectator active ngay; Horse Owner & Jockey chờ Admin duyệt.
    - 🔒 **Spectator chỉ tạo được ở đây** (✅ 2026-07-27): `RegisterCommandHandler` bootstrap **cả bộ** — `Spectator` row + `PointWallet` 100 điểm + `WalletTransaction` `Type="Initial"`. `POST /api/users` (Admin) **chỉ** insert mỗi `User`, nên tạo Spectator qua đường đó ra tài khoản **không có ví**: đăng nhập được nhưng mọi thao tác Flow 7 (xem số dư, đặt cược) đều vỡ. `CreateUserCommandHandler` nay **tra `Roles` theo `RoleId` → so `Code == "SPECTATOR"`** (không hardcode id, vì mapping `RoleId ↔ Code` phụ thuộc thứ tự seed — `RegisterCommandHandler` hardcode `SPECTATOR = 4` trong khi `DatabaseSeeder` insert theo thứ tự khác) và ném 400 *"Spectator accounts must be created via self-registration (/register), not by Admin."* Admin vẫn tạo được **Horse Owner / Jockey / Referee / Admin** như cũ. Tác dụng phụ có chủ đích: `roleId` không tồn tại nay trả 400 *"RoleId is invalid."* thay vì để rơi xuống FK violation → 409.
...
- **Flow 2 — Mời nài & nộp Entry** ✅ **đã hoàn thiện orchestration** (2026-06-25).
  - **Mời nài** (`POST /api/jockey-invitations` [HORSE_OWNER]): validate horse `Approved` + thuộc owner, race `Scheduled`, **registration đang mở** (✅ 2026-07-27), nài role JOCKEY + có `LicenseNumber`+`Weight`, chống trùng invitation active `(jockey+horse+race)`. HorseOwnerId lấy từ JWT.
    - 🆕 **Cửa đăng ký (2026-07-27):** trước đây handler **chỉ** check `race.Status == "Scheduled"` — race Scheduled mà Admin chưa bấm Open Registration (`RegistrationOpenAt == null`) vẫn mời nài được, tới bước `POST /entries` mới bị chặn ⇒ đẻ ra invitation `Pending`/`Accepted` cho race chưa nên nhận thao tác gì, và **lệch hẳn với `CreateEntry`**. Nay dùng **đúng cùng điều kiện + đúng cùng message** với `CreateEntryCommandHandler`: `RegistrationOpenAt == null` → *"Registration has not been opened."*; `RegistrationCloseAt <= now` → *"Registration is closed."* ⇒ **Open Registration nay là bước bắt buộc trước khi mời nài** (TEST_PLAN đã thêm bước 5d/5e).
...
> | 2 | **Gom logic hủy race** vào `IRaceLifecycleCoordinator.CancelRaceAsync(raceId, reason, throwOnFailure)` — pessimistic lock + transaction; cascade Entry `Pending`/`Approved` → **`Withdrawn`** (ghi `RejectionReason`), JockeyInvitation `Pending`/`Accepted`/`Confirmed` → **`Cancelled`**. `DeleteRaceCommandHandler` nay chỉ là wrapper gọi hàm này (`throwOnFailure: true`). ✅ **Hoàn cược đầy đủ** từ 2026-07-29 (T-28). | `RaceExecution/RaceLifecycleCoordinator.cs`, `Races/DeleteRace/DeleteRaceCommandHandler.cs` |
...
>
> **⚠️ Nợ kỹ thuật còn lại:** `ICurrentUser` **vẫn bị comment toàn bộ** (T-07); tie-break cuối "quyết định Admin" chưa có endpoint (T-10); FluentValidation hoãn (T-12); `GET /api/users` mở cho mọi role (T-26-cũ). ✅ **T-27 + T-28 đã đóng 2026-07-29.**
...
**⑪ Không đóng được giải khi race con còn sống** *(thêm 2026-07-27, `UpdateTournamentCommandHandler`)* — vá lỗ hổng cùng đợt với ⑩.
- **Vấn đề:** `PUT /api/tournaments/{id}` ghi đè `Status` **vô điều kiện** — không join sang `Races`, không cascade gì (khác hẳn `CancelRaceAsync` vốn cascade Entry→`Withdrawn`, Invitation→`Cancelled`). Vòng đời Race **hoàn toàn độc lập** với `Tournament.Status`: start, nhận cược, khóa odds, publish đều chỉ đọc `Race.Status`/`Entry.Status`, **không handler nào đọc `Tournament.Status`**. Hệ quả: set giải về `Cancelled` trong lúc có race `InProgress` thì race đó **vẫn chạy tiếp, vẫn nhận cược, vẫn publish + trả thưởng bình thường** — không crash, chỉ sai lệch hiển thị/báo cáo (UI báo "Đã hủy" trong khi tiền vẫn chạy).
- **Cách vá:** chặn ở cổng vào. Khi status **đổi sang** terminal (`TournamentStatus.IsTerminal(status)` **và** khác `tournament.Status` hiện tại), query `Races` của giải, lọc `Status ∉ {Finished, Cancelled}` (tức còn `Scheduled`/`InProgress`/`Paused`/`PendingResult`). Còn race nào → **400** kèm số lượng + tối đa 5 race đầu (`#id name (status)`): *"Tournament cannot be set to X while N race(s) are still active: … Finish or cancel those races first."*
- **Cố ý KHÔNG cascade** (đã cân nhắc rồi loại): hủy giải mà tự hủy luôn race con là xóa suất đua + lời mời hàng loạt chỉ vì một thao tác ở cấp giải. Hủy race là hành động riêng, có ràng buộc riêng (`CancelRaceAsync` chỉ nhận race `Scheduled`). *(Lý do cũ — "sẽ kéo lỗi hủy race không hoàn cược lan lên cấp giải" — nay không còn: T-28 đã đóng, `CancelRaceAsync` hoàn cược đầy đủ. Quyết định không cascade vẫn giữ vì lý do đầu.)*
- **Chỉ bắt lúc ĐỔI sang terminal:** sửa tên/mô tả của giải vốn đã `Finished`/`Cancelled` (FE gửi lại nguyên status cũ) vẫn phải chạy được — nếu so mỗi `IsTerminal(status)` thì giải đã đóng sẽ không bao giờ sửa nổi chữ nào nữa.
- ⚠️ **Worker `ProcessDueTournamentStatuses` KHÔNG bị guard này** — cố ý. Worker đẩy `Finished` theo **ngày** (`today > EndDate`); chặn nó lại sẽ khiến giải có một race `PendingResult` chưa ai publish **kẹt ở `Ongoing` vĩnh viễn**. Nghĩa là đường auto vẫn tạo được cảnh "giải Finished, race con còn sống" — chỉ khác là do quá hạn thật chứ không do Admin bấm nhầm. Muốn bịt nốt thì phải quyết xem giải quá hạn mà race chưa publish nên xử lý ra sao (chưa làm).
- ⚠️ **`DELETE /api/tournaments/{id}` vẫn hard-delete không guard** — `DeleteTournamentCommandHandler` `Remove()` thẳng row, không kiểm race con (ngoài phạm vi đợt này).
...
- **Flow 4 blind:** `GET {id}/legs/{i}/referee-view` (ẩn input referee kia đến khi cả hai submit), `PUT {i}/draft` (**persist thật** — upsert vào `LegRefereeDraft`, có `MyDraftData` để khôi phục nháp), `POST {i}/submit` (append-only, **`ValidatePositions`** trước, so khớp → Confirmed/AutoMatched + tính **Leg Points tuyến tính `N−p+1`** & LegOfficialResult, hoặc Conflicted + Paused; hết leg → PendingResult). Submit đầu tiên set `leg.Status = AwaitingSecondReferee`.
  - 🆕 **`Leg.StartedAt` nay được set khi leg THỰC SỰ bắt đầu** (2026-07-29): leg 1 ← `StartRaceAsync`; leg kế tiếp ← lúc leg trước được chốt (`AdvanceRaceIfComplete` ở `SubmitLegResult`, và nhánh resume của `OverrideLegResult`). Trước đây nó chỉ được ghi ở `SubmitLegResult` lúc referee **đầu tiên nộp bài** — tức **sau khi** chặng đã chạy xong — nên trang Live của spectator đứng ở *"Not started"* suốt cả chặng đang đua. Dòng `if (leg.StartedAt is null) leg.StartedAt = now;` trong `SubmitLegResult` giữ lại làm fallback cho race cũ. **Không** logic nghiệp vụ nào đọc `StartedAt` (chỉ `GetRaceLive` + `GetLegDetail` để hiển thị), nên đổi thời điểm ghi là an toàn. ❌ **Không còn settle prediction ở đây** — đã quay về Publish.
...
- **Flow 7 (cược RACE-LEVEL — sau revert 2026-07-25):** mỗi prediction cược **1 Entry về 1st của cả Race**. Routes ở `PredictionsController` (prefix `api/predictions`):
  ...
  **Vòng đời thực tế:** `Pending` (khi đặt) → `Locked` (khi `StartRaceAsync` chạy — T-20) → `Won`/`Lost` (khi Publish); hoặc `Cancelled` (hủy tay / revoke ngựa / lock account). Unpublish rollback đưa `Won`/`Lost` về `Locked`.
  ...
  ✅ **Hủy race HOÀN 100% điểm cược** (T-28, 2026-07-29) — `CancelRaceAsync` gọi `RefundPredictionsForCancelledRaceAsync` trong cùng transaction: prediction `Pending`/`Locked` → `Cancelled` + `CancelledAt`, cộng lại ví, ghi `WalletTransaction{Type="BetRefund"}`. Ví `IsFrozen` thì vẫn `Cancelled` nhưng **không** cộng tiền (đóng băng ví là cố ý chặn mọi biến động số dư). Phủ **cả** `DELETE /api/races/{id}` lẫn worker auto-cancel vì cả hai đi chung coordinator.
  Migrations liên quan: `UpdatePredictionSingleEntryBet`, `UpdatePredictionDelete`, `CompleteFlow7PredictionBetting`, `RevertAllToOriginalState` + `DropOrphanPerLegColumns` (cùng drop cột mồ côi per-leg; **cả hai nay đều idempotent** — T-27 đã đóng).
...
- **Còn lại (xem [.claude/TASKS.md](../.claude/TASKS.md) để có cách sửa):**
  - ✅ **Đã đóng 2026-07-29:** 2 migration drop trùng nay cùng dùng raw SQL `IF EXISTS` (**T-27**); `CancelRaceAsync` **đã hoàn cược** cho spectator (**T-28**).
...
- **23 migration**, tất cả ở `Infrastructure/Migrations/` (namespace `Infrastructure.Migrations`). HEAD hiện tại: **`20260729013601_DropTournamentLogoUrl`** (drop `Tournaments.LogoUrl` — gỡ hẳn logo giải khỏi cả FE lẫn BE; `Up()` mất dữ liệu, `Down()` chỉ dựng lại cột rỗng). Trước đó: `20260728165812_SimplifyOddsToSingleNumber` (drop `Entries.PublishedOdds`, `Races.OddsPublishedAt`, `Races.BettingLockedAt` — xem khối "ĐỢT 2026-07-28 (đợt 2)" ở mục 6). Trước đó có hai migration **cùng drop** bộ cột mồ côi per-leg (`Predictions.LegNumber`/`LegRaceId` + FK/index, `Legs.ExecutionStatus`/`PredictionOpenedAt`/`PredictionClosedAt`, `Horses.Stamina`):
  - `20260725113128_RevertAllToOriginalState`
  - `20260725162506_DropOrphanPerLegColumns`
  ✅ **T-27 đã đóng (2026-07-29):** trước đây bản `RevertAllToOriginalState` dùng `DropForeignKey`/`DropIndex`/`DropColumn` **không** có `IF EXISTS`, nên DB nào từng chạy app ở khoảng commit `e490dc8`…`eec39fb` (apply bản raw-SQL trước) sẽ **crash lúc khởi động** khi EF chạy nốt bản còn lại. Nay `Up()` của nó đã viết lại thành **raw SQL `IF EXISTS`** copy nguyên xi bản kia ⇒ hai migration idempotent với nhau, thứ tự nào cũng chạy được.
  ⚠️ `Down()` của `RevertAllToOriginalState` **giữ nguyên** — nó dựng lại `LegNumber` NOT NULL + FK sang `Legs` nên rollback ngược tới mốc này vẫn có thể fail nếu `Predictions` đã có dữ liệu.
